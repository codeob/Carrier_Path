  ...
  const handleApply = async (data) => {
    try {
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        setTimeout(() => navigate('/user/auth'), 2000);
      }
    } finally {
      ...
      <div className="max-w-7xl mx-auto ${showApplyModal || showSuccessModal ? 'backdrop-blur-md' : ''}">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Available Jobs</h1>
              <p className="text-gray-600">Browse and apply for open job positions</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 mb-6">
              ...
              <div key={job._id} className="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200 p-4 max-w-md">
                ...
                <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-200">
                  <button
                    ...
                    className="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-medium py-1 px-2 rounded-md transition duration-200 flex items-center gap-1 text-sm"
                  >
                    <span>📩</span>
                    Apply
                  </button>
        ...
        {showApplyModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <form onSubmit={handleSubmit(handleApply)} className="space-y-4" encType="multipart/form-data">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Full Name *</label>
                  <input
                    type="text"
                    {...register('fullName', { required: 'Full name is required' })}
                    className={`w-full p-2 border rounded-md focus:ring-2 focus:ring-indigo-500 text-sm ${errors.fullName ? 'border-red-300' : 'border-gray-300'}`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full p-2 border rounded-md focus:ring-2 focus:ring-indigo-500 text-sm ${errors.email ? 'border-red-300' : 'border-gray-300'}`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full p-2 border rounded-md focus:ring-2 focus:ring-indigo-500 text-sm ${errors.resume ? 'border-red-300' : 'border-gray-300'}`}
                  />
        ...
        {showSuccessModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md text-center">
              ...
              <p className="text-gray-600 mb-4">Your application for {selectedJob?.title} has been successfully submitted.</p>
              <button
                onClick={() => {
                  setShowSuccessModal(false);
                  setSelectedJob(null);
