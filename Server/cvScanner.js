...
const mongoose = require('mongoose');
  ...
  const NotificationSchema = new mongoose.Schema(
    {
      ...
      type: { type: String, required: true }, // e.g., 'application_status'
  ...
  module.exports = mongoose.model('Notification', NotificationSchema);const Notification = require('../models/Notification');

  async function createApplicationStatusNotification({ userId, applicationId, status, jobId, jobTitle }) {
    ...
    const message =
      normalizedStatus === 'accepted'
        ? `Congratulations! Your application for "${jobTitle || 'the job'}" was accepted.`
        : normalizedStatus === 'rejected'
        ? `Your application for "${jobTitle || 'the job'}" was not successful this time.`
        : `Your application for "${jobTitle || 'the job'}" was updated to: ${status}.`;
    ...
    return Notification.create({
      ...
      title,
      message,
      data: { applicationId, status: normalizedStatus, jobId, jobTitle },
      read: false
    });
  }

  async function listUserNotifications(userId, { page = 1, limit = 20, unreadOnly = false } = {}) {
    const query = { userId };
  ...
  const Application = require('../models/Application'); // Adjust to your actual path/model
  ...
  async function updateApplicationStatus(req, res) {
    try {
      ...
      const allowed = ['accepted', 'rejected', 'under_review', 'shortlisted', 'on_hold'];
      const normalizedStatus = String(status).toLowerCase();
      ...
      if (recruiterId && String(application.recruiterId) !== String(recruiterId)) {
        return res.status(403).json({ error: 'Not authorized to update this application.' });
      }

      await Application.updateOne(
        { _id: applicationId },
        { $set: { status: normalizedStatus, statusUpdatedAt: new Date() } }
      );
      ...
      await createApplicationStatusNotification({
        ...
        jobTitle: application.jobId?.title || application.jobTitle || 'the job'
      });
  ...
  async function deleteApplications(req, res) {
    try {
      ...
      if (all === true) {
        // Delete all applications created for this recruiter
        const result = await Application.deleteMany({ recruiterId });
        return res.json({ success: true, deletedCount: result.deletedCount || 0 });
      }

      if (!Array.isArray(ids) || ids.length === 0) {
        return res.status(400).json({ error: 'Provide ids array or set all=true to delete all.' });
      }

      // Filter invalid IDs to avoid cast errors
      ...
      console.error('deleteApplications error:', err);
  ...
  const { listUserNotifications, markNotificationsRead } = require('../services/notificationService');
  // const { requireAuth } = require('../middleware/auth'); // plug your middleware
  ...
  router.get('/', /* requireAuth, */ async (req, res) => {
    try {
      ...
      const data = await listUserNotifications(userId, { page, limit, unreadOnly });
      res.json(data);
    } catch (err) {
  ...
  router.post('/read', /* requireAuth, */ async (req, res) => {
    try {
      ...
      if (!userId) return res.status(401).json({ error: 'Unauthorized' });
      const { ids } = req.body || {};
      const result = await markNotificationsRead(userId, Array.isArray(ids) ? ids : []);
